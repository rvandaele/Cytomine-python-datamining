...
if __name__ == "__main__":
	...
	parameters = {
		...
		'model_NT_P2':None,
		'model_F_P2': None,
		'model_filter_size': None,
		'model_beta': None,
		'model_n_iterations': 0,
		'model_ncandidates': 0,
		'model_sde': 0,
		'model_T': 0,
		'model_name': '',
	...
	parameters['model_NT_P1'] = options.model_NT_P1
	parameters['model_F_P1'] = options.model_F_P1
	parameters['model_R_P1'] = options.model_R_P1
	parameters['model_sigma'] = options.model_sigma
	parameters['model_delta'] = options.model_delta
	parameters['model_P'] = options.model_P
	parameters['model_R_P2'] = options.model_R_P2
	parameters['model_ns_P2'] = options.model_ns_P2
	parameters['model_NT_P2'] = options.model_NT_P2
	parameters['model_F_P2'] = options.model_F_P2
	parameters['model_filter_size'] = options.model_filter_size
	parameters['model_beta'] = options.model_beta
	parameters['model_n_iterations'] = options.model_n_iterations
	parameters['model_ncandidates'] = options.model_ncandidates
	parameters['model_sde'] = options.model_sde
	...
	job_parameters['model_NT_P1'] = parameters['model_NT_P1']
	job_parameters['model_F_P1'] = parameters['model_F_P1']
	job_parameters['model_R_P1'] = parameters['model_R_P1']
	job_parameters['model_sigma'] = parameters['model_sigma']
	job_parameters['model_delta'] = parameters['model_delta']
	job_parameters['model_P'] = parameters['model_P']
	job_parameters['model_R_P2'] = parameters['model_R_P2']
	job_parameters['model_ns_P2'] = parameters['model_ns_P2']
	job_parameters['model_NT_P2'] = parameters['model_NT_P2']
	job_parameters['model_F_P2'] = parameters['model_F_P2']
	job_parameters['model_filter_size'] = parameters['model_filter_size']
	job_parameters['model_beta'] = parameters['model_beta']
	job_parameters['model_n_iterations'] = parameters['model_n_iterations']
	job_parameters['model_ncandidates'] = parameters['model_ncandidates']
	job_parameters['model_sde'] = parameters['model_sde']
	...
	if(parameters['cytomine_training_images']=='all'):
		...
		tr_im = [int(p) for p in parameters['cytomine_training_images'].split(',')]

	(dataset,rep,img,feature_offsets_1) = build_phase_1_model(repository,tr_image=tr_im,image_ids=ims,n_jobs=parameters['model_njobs'],NT=parameters['model_NT_P1'],F=parameters['model_F_P1'],R=parameters['model_R_P1'],sigma=parameters['model_sigma'],delta=parameters['model_delta'],P=parameters['model_P'],X=Xc,Y=Yc)
	clf = LonelyTrees(n_estimators=parameters['model_NT_P1'],n_jobs=parameters['model_njobs'])
	clf = clf.fit(dataset,rep)

	joblib.dump(clf, '%s%s_classifier_phase1.pkl' % (model_repo, parameters['model_name']))
	joblib.dump(feature_offsets_1,'%s%s_offsets_phase1.pkl'%(model_repo,parameters['model_name']))

	print "PHASE 1 DONE MY MAN"
	dataset = None
	rep = None
	img = None
	clf = None
	...
	for id_term in term_list:
		(dataset, rep, number, feature_offsets_2) = build_phase_2_model(repository,tr_image=tr_im, image_ids=ims, n_jobs=parameters['model_njobs'], IP=id_term, NT=parameters['model_NT_P2'], F=parameters['model_F_P2'],R=parameters['model_R_P2'], N=parameters['model_ns_P2'], sigma=parameters['model_sigma'], delta=parameters['model_delta'])
		reg = LonelyTreesRegressor(n_estimators=parameters['model_NT_P2'], n_jobs=parameters['model_njobs'])
		reg.fit(dataset, rep)
		joblib.dump(reg,'%s%s_donner_regressor_phase2_%d.pkl'%(model_repo,parameters['model_name'],id_term))
		joblib.dump(feature_offsets_2, '%s%s_donner_offsets_phase2_%d.pkl' % (model_repo, parameters['model_name'], id_term))

	print "PHASE 2 DONE MY MAN"
	...
	edges = build_edgematrix_phase_3(X, Y, parameters['model_sde'], parameters['model_delta'], parameters['model_T'])
	joblib.dump(edges,'%s%s_edgematrix_phase3.pkl'%(parameters['model_save_to'],parameters['model_name']))

	F = open('%s%s_donner_parameters.conf'%(parameters['model_save_to'],parameters['model_name']),'wb')
	F.write('cytomine_id_terms %s\n'%parameters['cytomine_id_terms'])
	F.write('model_njobs %d\n'%parameters['model_njobs'])
	F.write('model_NT_P1 %d\n'%parameters['model_NT_P1'])
	F.write('model_F_P1 %d\n'%parameters['model_F_P1'])
	F.write('model_R_P1 %d\n'%parameters['model_R_P1'])
	F.write('model_sigma %f\n'%parameters['model_sigma'])
	F.write('model_delta %f\n'%parameters['model_delta'])
	F.write('model_P %f\n'%parameters['model_P'])
	F.write('model_R_P2 %d\n'%parameters['model_R_P2'])
	F.write('model_ns_P2 %d\n'%parameters['model_ns_P2'])
	F.write('model_NT_P2 %d\n'%parameters['model_NT_P2'])
	F.write('model_F_P2 %d\n'%parameters['model_F_P2'])
	F.write('model_filter_size %d\n'%parameters['model_filter_size'])
	F.write('model_beta %f\n'%parameters['model_beta'])
	F.write('model_n_iterations %d\n'%parameters['model_n_iterations'])
	F.write('model_ncandidates %d\n'%parameters['model_ncandidates'])
	F.write('model_sde %f\n'%parameters['model_sde'])
	F.write('model_T %d'%parameters['model_T'])
	F.close()
	print "PHASE 3 DONE MY MAN"