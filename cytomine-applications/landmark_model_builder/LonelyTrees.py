...
def build_lonely_tree(X,y,max_features,max_depth,min_samples_split):
	clf = ExtraTreeClassifier(max_features=max_features,max_depth=max_depth,min_samples_split=min_samples_split)
	clf = clf.fit(X,y)
	return clf
	
def lonelytree_training_mp_helper(jobargs):
	return build_lonely_tree(*jobargs)
	
def lonelytree_test_mp_helper(jobargs):
	return test_lonely_tree(*jobargs)

def test_lonely_tree(tree,X):
...
class LonelyTrees:
	...
	def fit(self,X,y):
		...
		self.trees = p.map(lonelytree_training_mp_helper,jobargs)
		p.close()
		p.join()
	...
	def predict_proba(self,X):
		...
		probas = p.map(lonelytree_test_mp_helper,jobargs)
		p.close()
		p.join()
...
if __name__ == "__main__":
	clf = LonelyTrees(n_estimators=32,max_features=2,n_jobs=4)
