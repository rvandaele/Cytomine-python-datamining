...
if __name__ == "__main__":
	...
	par = {}
	F = open('%s%s_lc_parameters.conf' % (parameters['model_load_from'], options.cytomine_model_name))
	...
	(h2, v2, h3, v3, sq) = joblib.load('%s%s_lc_feature_map.pkl' % (model_repo, options.cytomine_model_name))
	for id_term in id_terms:
		reg = joblib.load('%s%s_lc_regressor_%d.pkl' % (model_repo, options.cytomine_model_name, id_term))
		print "term %d" % id_term
		for id_img in pr_im:
			print "img %d" % id_img
			vote_map = build_vote_map(repository, id_img, reg, h2, v2, h3, v3, sq, int(par['model_step']))
			np.savez_compressed(
				'%slc_votemap-%s-%d-%d.npy' % (repository, options.cytomine_model_name, id_img, id_term), vote_map)
	(h, w) = vote_map.shape
	vote_map = None
	...
	(mu, P, mx, my) = joblib.load('%s%s_lc_pca.pkl' % (model_repo, options.cytomine_model_name))
	hash_error = {}
	...
	for id_img in pr_im:
		...
		for i in range(id_terms.size):
			global_vote_map[:, :, i] = \
				np.load('%slc_votemap-%s-%d-%d.npy.npz' % (repository, options.cytomine_model_name, id_img, id_terms[i]))[
					'arr_0']
		...
		while (current_R >= R_min):
			print current_R, type(current_R)
			print np.round(current_R)
			print int(np.round(current_R))
			print find_best_positions(global_vote_map, coords, int(np.round(current_R)))
	...
	job_parameters = {'cytomine_models': parameters['cytomine_model_name'],
